...
def check_config_file(args):
    if not os.path.exists(args.config_file):
        print("<WARNING> Configuration file not found: {args.config_file}")
        print("<WARNING> Only argument and default config will be used")
        if args.config_file != DEFAULT_CONFIG_FILE:
            print("<WARNING> You can use --config-file option for another config file")
...
def args_parse(args_liner):
    ...
    parser.add_argument('-f', '--config-file', help='Config file', default='/etc/django_utils/config')
    ...
    parser.add_argument('-c', '--check', action="store_true", help='Check web server status [Start if down]')
    ...
    parser.add_argument('--dump-file', required=re.search('--db-restore', args_liner), help='Dump File to Restore [.dump(.xz)]')
    ...
    parser.add_argument('--media-file', required='--media-restore' in sys.argv, help='Media File to Restore [.tar(.xz)]')
...
def main():
    ...
    os.chdir(cfg["global"]["project_path"])
    # Init Script logger #
    log_file = logger_init(args, cfg)
    logger.info(f'[script] o-> Start (Env:{args.env}) <-o')
    # File Extension + Existing Path Check #
    if not args.db_restore and not args.media_restore: 
